...
public class OrderDAOImpl implements OrderDAO {
    ...
    @Override
    public void increaseProductStock(Integer orderId, Connection conn) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (rowsAffected == 0) {
                throw new SQLException("未找到订单或更新库存失败，orderId=" + orderId);
            }
        }
    }

    @Override
    public void updateStatus(Integer orderId, String status, Connection conn) throws SQLException {
    ...
    @Override
    public void reduceProductStock(Integer orderId, Connection conn) throws SQLException {
        // 查询订单中的所有订单项
        String orderItemsSql = "SELECT product_id, quantity FROM order_item WHERE order_id =?";
        try (PreparedStatement orderItemsStmt = conn.prepareStatement(orderItemsSql)) {
            orderItemsStmt.setInt(1, orderId);
            ResultSet rs = orderItemsStmt.executeQuery();
            ...
            try (PreparedStatement updateStmt = conn.prepareStatement(
                ...
                while (rs.next()) {
                    ...
                    int quantityToReduce = rs.getInt("quantity");
                    ...
                    try (PreparedStatement checkStmt = conn.prepareStatement(checkStockSql)) {
                        ...
                        if (stockRs.next() && stockRs.getInt("quantity") >= quantityToReduce) {
                            // 设置参数并添加到批处理
                            updateStmt.setInt(1, quantityToReduce);
                            updateStmt.setInt(2, productId);
                            updateStmt.addBatch();
                        } else {
                            throw new SQLException("商品ID: " + productId + " 库存不足，无法发货");
                        }
                    }
                }
